...
function Subnet(props: SubnetProps): JSX.Element{
    ...
    return(
        <button value={subnetId}
            ...
            className={clsx(classes,'rounded-lg','w-40','h-20','m-2','flex','flex-col','justify-center','items-center')}>
            {subnetType === 'public'
                ? <Icon className={"relative top-0 right-0 float-right"}
                    icon="ant-design:lock-outlined"
                    width="25" height="25"
                    fr={undefined}/>
                : undefined}
            <div className={"font-bold"}>{tagName.split(' ')[0]}</div>
            <div className={"font-bold"}>{tagName.replace('Public','').replace('private','')}</div>
            <div>({cidrBlock})</div>
        </button>
    );
}
