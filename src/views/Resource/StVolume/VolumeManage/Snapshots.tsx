            ...
            {snapshotList.map(snapshot =>
                <div className="flex justify-between w-96 border-b-2" key={snapshot}>
                    ...
                    <DropdownMenu>
                        ...
                        <DropdownMenuContent>
                            <DropdownMenuItem className="text-red-600" onClick={() => console.log('delete snapshot')}>Delete</DropdownMenuItem>
                        </DropdownMenuContent>
                    </DropdownMenu>
                </div>)}
            {creating
                ? <WithEdit visible={creating} onOk={() => setCreating(false)} onCancel={() => setCreating(false)}>
                    <div className="mb-2">Give your snapshot a name.</div>
                    <Input type="text" className="w-96" value={newSnapshotName} onChange={e => setNewSnapshotName(e.target.value)} />
                </WithEdit>
                : <button onClick={() => setCreating(true)} className="inline text-yellow-550">
                    <Icon icon="carbon:add" className="inline-block mx-1" width="15" height="15" />
